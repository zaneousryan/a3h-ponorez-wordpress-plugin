			...
			for($i=1; $i<=20; $i++){
				...
				if($i==1){
					if (isset($guestTypes[0]) && $guest == $gID) {
						$html .= do_shortcode('[loadPonorezActivityGuestField id="'.$gID.'" name="'.$gType.' (13yrs +): $109.00 + taxes & fee" min="'.$gMin.'" max="'.$gMax.'"]');
					}
				}elseif($i==2){
					if (isset($guestTypes[0]) && $guest == $gID) {
						$html .= do_shortcode('[loadPonorezActivityGuestField id="'.$gID.'" name="'.$gType.' (4-12yrs): $84.00 + taxes & fee" min="'.$gMin.'" max="'.$gMax.'"]');
					}
				}elseif ($i==3) {
					if (isset($guestTypes[0]) && $guest == $gID) {
